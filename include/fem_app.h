...
void print_debug(taichi::lang::vulkan::VkRuntime& vulkan_runtime,
                 taichi::lang::DeviceAllocation& alloc, int it,
                 bool use_int = false) {
...
class FemApp {
 public:
  void run_init(int _width, int _height, std::string path_prefix,
    ...
    device = static_cast<taichi::lang::vulkan::VulkanDevice*>(
    ...
    loaded_kernels.ndarray_to_ndarray_kernel =
        module->get_kernel("ndarray_to_ndarray");
    ...
    loaded_kernels.update_beta_r_2_kernel =
        module->get_kernel("update_beta_r_2");
    ...
    devalloc_alpha_scalar =
        vulkan_runtime->get_ti_device()->allocate_memory(alloc_params);
    devalloc_beta_scalar =
        vulkan_runtime->get_ti_device()->allocate_memory(alloc_params);

    load_data(vulkan_runtime.get(), devalloc_indices, indices_data,
              sizeof(indices_data));
    load_data(vulkan_runtime.get(), devalloc_c2e, c2e_data, sizeof(c2e_data));
    load_data(vulkan_runtime.get(), devalloc_vertices, vertices_data,
              sizeof(vertices_data));
    load_data(vulkan_runtime.get(), devalloc_ox, ox_data, sizeof(ox_data));
    load_data(vulkan_runtime.get(), devalloc_edges, edges_data,
              sizeof(edges_data));
  ...
  void run_render_loop(float a_x = 0, float a_y = -9.8, float a_z = 0) {
    ...
    cmd_list->begin_renderpass(
        ...
        /*ymax=*/height, /*num_color_attachments=*/1, &image, &color_clear,
        &clear_colors, &depth_allocation,
        /*depth_clear=*/true);

    RenderConstants* constants =
        (RenderConstants*)device->map(render_constants);
    constants->proj = glm::perspective(
        glm::radians(55.0f), float(width) / float(height), 0.1f, 10.0f);
